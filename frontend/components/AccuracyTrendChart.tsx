...
export default function AccuracyTrendChart({ points }: { points: Point[] }) {
  ...
  const yFor = (accuracy: number) => PAD_TOP + (1 - accuracy / 100) * plotHeight;

  const linePath = points
    .map((p, i) => `${i === 0 ? "M" : "L"} ${xFor(i)} ${yFor(p.accuracy)}`)
    .join(" ");
      ...
      {hovered && (
        <div
          ...
          style={{
            ...
            top: `${(yFor(hovered.accuracy) / HEIGHT) * 100}%`,
            transform: "translate(-50%, -130%)",
          }}
        >
          {formatDate(hovered.date)} · {hovered.accuracy}%
        </div>
      )}
    </div>
  );
